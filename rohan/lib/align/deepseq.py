...
def get_coverage(aligned,reference2seq,target_ini,target_end,target=False,ref_name=None):
    ...
    if target:
        ...
        for pileupcol in aligned.pileup(ref_name, target_ini,target_end,max_depth=10000000):
...
#         -n {reflen} -m {reflen}
    ...
    coms+=[f'fastp -i {dirp}/R1.fastq -I {dirp}/R2.fastq -o {dirp}/R1_flt.fastq -O {dirp}/R2_flt.fastq -A -q {quality_coff} -e {quality_coff} -j {dirp}/R2_flt.fastq.report.json -h {dirp}/R2_flt.fastq.report.html 2> {dirp}/log_fastp.log' if method=='local' else f'fastp -i {dirp}/R.fastq -o {dirp}/R_flt.fastq -A -q {quality_coff} -e {quality_coff} -j {dirp}/R_flt.fastq.report.json -h {dirp}/R_flt.fastq.report.html 2> {dirp}/log_fastp.log',
...
def get_daligned(dirp,method,target=False):
    """
    ...
    reference2seq=read_fasta(f'{dirp}/reference.fasta')
    # target_region
    for ref in [k for k in reference2seq if not 'target' in k]:
        logging.info(ref)
        cfg={}
        if not target:
            cfg['target_ini']=0
